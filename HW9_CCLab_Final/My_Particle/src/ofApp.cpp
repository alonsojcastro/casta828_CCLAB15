//
//  Created by Alonso Castro 11/25/2015
//  Modified from Dorkshop OpenFrameworks by Umi Syam
...
void ofApp::setup(){
    ...
    //sound
    komodo.load("Komodo_Concept _1.mp3");
...
//            }
        ...
        // find contours which are between the size of 20 pixels and 1/3 the w*h pixels. (340*240)/3 - sample
...
//        cursors.clear() ;
        ...
        for ( int i = 0 ; i < contourFinder.nBlobs ; i++ ){
            ...
            cout<<"kinect width: "<< kinect.width<<std::endl;
            cout<<"kinect height: "<< kinect.height<<std::endl;
...
//                ofVec2f positionofeachblob =  contourFinder.blobs[i].centroid;
                for(i = 0; i < p.size(); i++){
                    p[i].update(::ofRandom(iCentroid.x-3, iCentroid.x+3), iCentroid.y);
//                    p[i].update(positionofeachblob.x, positionofeachblob.y);
                }
//            }
...
//            cursors.push_back( iCentroid ) ;
            ...
            ofVec2f tempPos =  contourFinder.blobs[i].centroid;
            
            int kinectToWindowWidth = ofMap(kinect.width, 0, 640, 0, ofGetWidth());
            int kinectToWindowHeight = ofMap(kinect.height, 0, 480, 0, ofGetHeight());
            
            
            boundaryX1 = kinectToWindowWidth;
            boundaryX2 = kinectToWindowWidth - 120;//520
            boundaryY1 = kinectToWindowHeight;
            boundaryY2 = kinectToWindowHeight - 70;//410
            cout << kinectToWindowWidth <<endl;
            
            cout<<"mouse x"<< mouseX<<std::endl;
            cout<<"mouse y: "<< mouseY<<std::endl;
            
            
            ofVec2f parPos = p[i].getPos();
            
            if ((parPos.x <= (boundaryX1) && parPos.x >= (boundaryX2)
                  && parPos.y <= (boundaryY1) && parPos.y >= (boundaryY2))){
                if ( contourFinder.blobs.size() > 0 ){
                    p[i].setMode(currentMode);
                    p[i].setAttractPoints(& cursors);
                    p[i].update(parPos.x, parPos.y);
                    komodo.play();
                }
...
//        grayImage.draw(220, 320, 400, 300);
//        contourFinder.draw(220, 320, 400, 300);
//        ofSetColor(255, 0, 0);
//        cout<<" x1 : "<<boundaryX1<<"x2: "<<boundaryX2<<"y1: "<<boundaryY1<<"y2: "<<boundaryY2<<std::endl;
//    

//        ofSetColor(255, 0, 0);
//        ofNoFill();
//        ofDrawRectangle(420, 250, 400, 300);
//      ofDrawRectangle(boundaryX1, boundaryY1,boundaryX2, boundaryY2);
//        ofFill();
